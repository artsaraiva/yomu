...
5. **Speed benchmark on model changes.** A change to a model, the inference runtime (llama.cpp) or `LlmModelCatalog` runs `scripts/run-speed-benchmark.sh` on a phone or emulator and pastes the printed table in the PR. The numbers are report-only; nothing gates on them.
