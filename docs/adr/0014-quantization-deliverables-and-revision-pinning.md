...
So `translation/hf/`, `ModelManager.downloadHfModel`, the settings tier-2 branch, and `LlmModelTier.HF_AUTH` are deleted. The registered OAuth client stays; the deleting commit is named from the follow-up issue so revival is a lookup rather than an excavation.
