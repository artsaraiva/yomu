...
So `translation/hf/`, `ModelManager.downloadHfModel`, the settings tier-2 branch, and `LlmModelTier.HF_AUTH` are deleted. The registered OAuth client stays, and so revival is a lookup rather than an excavation: the path was deleted in `df8af305fce8f1a391cbce3ea0e8a20163b2046a` (#187). Check out its parent for the auth manager, token store and downloader. `LlmModelTier.HF_AUTH` itself outlived that commit and goes with the tier collapse.
