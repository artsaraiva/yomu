...
> **Revised in part by [ADR-0010](0010-qwen-default-cat-demoted-to-floor.md).** This ADR kept 0.8b as the safe default "pending phone confirmation" and deferred any default change to the ADR-0008 promote trigger. The [#72](https://github.com/artsaraiva/yomu/issues/72) confirmation run (2026-08-18) supplied that evidence — Qwen2.5-1.5B is ~4× cleaner than 0.8b at speed parity inside the same budget — so ADR-0010 exercises the trigger and makes **Qwen2.5-1.5B-Instruct** the default; 0.8b becomes the low-storage floor. The selectable-set structure, three delivery tiers, floors, and custom slot below all stand.

> **Revised in part by [ADR-0017](0017-experimental-curated-tier.md).** Two clauses change. **"A small curated shortlist"** is no longer small: the catalog goes from 3 deliverables to 19, with a ceiling of about 9B parameters enforced by the existing fit gate, which shows the 9B tier only on 12 GB+ phones. **Evidence-gated membership** is relaxed into a tier rather than dropped: a deliverable may now join on desk research, tagged **Experimental** until a per-deliverable phone check takes one of ADR-0017's three exits. ADR-0017 also reopens the licence question this ADR's tiering raised (Apache-2.0 or MIT only) and admits uncensored deliverables. The default that is never gated out, device gating, and the custom slot stand.

> **Superseded in part by [ADR-0014](0014-quantization-deliverables-and-revision-pinning.md).** The three-tier delivery split below collapses to one tier: the HuggingFace-authenticated tier 2 never had a member, never had a candidate, and never worked (no registered OAuth redirect), so it is deleted. "Licence sorts a model into a tier; it does not exclude it" no longer holds — a deliverable Yomu cannot redistribute is not curated. ADR-0014 also settles what #140 asked of this ADR: a catalog entry is one model at one quantization, the device-fit gate downgrades to a smaller quantization instead of hiding, and pinning stays content-addressed. The selectable-set core — curated shortlist, default never gated out, evidence-gated membership, the custom slot — stands.

The [#84](https://github.com/artsaraiva/yomu/issues/84) bake-off supplied the measured ranking [ADR-0008](0008-translation-model-selection.md) said did not exist, and it changes the answer. On the [ADR-0004](0004-translation-eval-contract.md) page-level path, several models beat the shipped CAT-Translate-0.8b, and the winners are **not** the CAT siblings that ADR-0008 and [#72](https://github.com/artsaraiva/yomu/issues/72) presumed — a general 2B instruct model leaves ~12× less Japanese residue than the 1.4b specialist (see the [#84 bake-off write-up](https://github.com/artsaraiva/yomu/blob/9bdad40c900eba73ecb9b3bfe829516caf25a5a4/eval/challenger-llm-bakeoff-84.md), removed with the eval code by [#232](https://github.com/artsaraiva/yomu/issues/232)).
