...
> **Revised by [#125](https://github.com/artsaraiva/yomu/issues/125) (2026-09-09).** The clause retaining the cache for the deterministic per-line engines — the floor engines, ML Kit and OPUS-MT — is superseded: the cache was deleted outright. ML Kit's translator is already local, and OPUS-MT has produced no scored line in any retained benchmark run; meanwhile the in-memory half was keyed by source text alone, so it corrupted per-engine benchmark numbers, and the persistent half was never wired into DI and never executed a line. The bypass reasoning above stands — it is now the whole behaviour, on every path. The empty `translation_cache` table is retained: dropping it forces `AppDatabase version = 4` under `fallbackToDestructiveMigration()`, which would wipe real translation history to remove a table that has never held a row.

> **Relocated by [ADR-0012](0012-page-translation-slot.md) (2026-09-09).** Page context, id addressing, per-bubble fallback, and empty-OCR omission stand. Model-specific prompt construction, strategy, parsing, budgets, and timeouts now live inside the selected translation slot; the pipeline projects geometry-bearing conversation blocks into the slot's geometry-free page input and applies the existing guard and fallback decisions.
...
## Amendment (#33): the class-label requirement is dropped

The requirement carried to #33 above — decide whether the detector exposes per-box speech / narration / sfx — is resolved as **no class label**, per [ADR-0007](0007-bubble-text-detection-approach.md). No deployable manga detector publishes more than one class (`Kiuyha/Manga-Bubble-YOLO` exposes a single `Text` class), the runtime never carried a class (`parseYoloDetections` hardcodes `label = "bubble"`), and #44 deleted the eval's per-class rows as fabricated, so nothing could score a class output if one existed. The page-level prompt therefore gets **no class markers**; narration and sfx are not distinguished from dialogue. Deriving the class heuristically at runtime was rejected as a fabricated field flowing into the prompt. The requirement paragraph above stays as written — it records the dependency that existed and why it died — rather than being edited out.

## Amendment (#53): the acceptance gate now has a contract

*(The gate is retired, never built: [ADR-0015](0015-quality-belongs-to-the-model.md) supersedes ADR-0006.)* The gate above was a pointer to a research note; its shape is now fixed by [ADR-0006](0006-coherence-gate-contract.md). The set is hand-authored **minimal pairs** (correct target vs. a single-referent-flipped corruption) scored by **contrastive accuracy** — the model assigns higher probability to the correct target. Passing is **directional**: accuracy with session context must beat accuracy with it blanked, on the same pairs (paired, chance floor 0.5), not an absolute bar. Scoring runs **off device** on desktop llama.cpp, so **this ADR owes no new native work** — the production path stays generation-only. The corpus and runner are post-map build work; the gate is defined now but cannot run until this ADR's architecture is built (`sessionContext` is still unread today, per #47).
...
**Consequence for the #53 amendment** *(moot: [ADR-0015](0015-quality-belongs-to-the-model.md) retires the gate)*. [ADR-0006](0006-coherence-gate-contract.md)'s directional gate — accuracy *with* session context against accuracy with it blanked — no longer has a cross-page arm to measure. It becomes runnable on **intra-page** coherence only.
